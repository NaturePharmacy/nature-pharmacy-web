...
function getHealthScore(seller: Seller): HealthScore {
  const rating = seller.sellerInfo?.rating ?? 0;
  const verified = seller.sellerInfo?.verified ?? false;

  if (rating >= 4.5 && verified) {
    return { label: 'Excellent', color: 'text-green-800', bg: 'bg-green-100' };
  }
  if (rating >= 3.5 || verified) {
    return { label: 'Bon', color: 'text-blue-800', bg: 'bg-blue-100' };
  }
  if (rating >= 2.5) {
    return { label: 'Moyen', color: 'text-yellow-800', bg: 'bg-yellow-100' };
  }
  return { label: 'À surveiller', color: 'text-red-800', bg: 'bg-red-100' };
}
...
export default function AdminSellers() {
  ...
  const t = {
    ...
    es: {
      ...
      totalSellers: 'Vendedores totales',
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredSellers.length === 0 ? (
                  ...
                  filteredSellers.map((seller) => {
                    const health = getHealthScore(seller);
                    return (
                      <tr key={seller._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-4 text-right text-sm text-gray-600">
                          {formatPrice(seller.sellerInfo?.totalSales || 0)}
                        </td>
                        ...
                        <td className="px-4 py-4 text-center">
                          <span
                            className={`px-2 py-1 text-xs font-medium rounded-full ${health.bg} ${health.color}`}
                          >
                            {health.label}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-4 text-center">
                          <button
                            onClick={() =>
                              toggleVerification(
                                seller._id,
                                seller.sellerInfo?.verified || false
                              )
                            }
