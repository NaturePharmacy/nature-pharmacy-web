...
import { mockPayPalCheckout } from '../../mocks/paypal.mock';

test.describe('Buyer Checkout - PayPal', () => {
  ...
  test('buyer can select PayPal payment', async ({ buyerPage }) => {
  ...
  test('PayPal mock intercepted', async ({ buyerPage }) => {
    const checkoutPage = new CheckoutPage(buyerPage);
    const address = TestData.shippingAddress();
